...
> Current install, privacy, release, and folder documentation: [PROJECT.md](PROJECT.md).
>
> The material below is the original development handoff/history and includes superseded OAuth-oriented notes. The current skin uses private Google iCal feeds instead.

## Session Handoff

Use this section as the persistent context snapshot for future Codex sessions on this project. At the end of each completed prompt, update the entries below so they always reflect the three most recent prompts, with the newest item at the top.

### Most Recent

- Date: 2026-03-31
- Prompt summary: Resumed the Rainmeter Google Calendar cleanup, confirmed the old `GoogleCalendar` and `GoogleCalendar Flyout` skin folders were gone, updated the project README to record that cleanup, and verified the renamed skins still work in Rainmeter.
- Status: Completed
- Next context: The renamed skins are stable. The next useful pass is manual Rainmeter verification and any follow-up rename cleanup for internal helper/config filenames such as `GoogleCalendar.config.json`.

### Previous

- Date: 2026-03-30
- Prompt summary: Built the Rainmeter Google Calendar skin prototype with a collapsible flyout, mock and cached event loading, PowerShell sync/auth helpers, a Windows setup GUI, and a Rainmeter setup button.
- Status: Completed with follow-up
- Next context: The main remaining checks are Rainmeter-side behavior after the folder rename, including flyout positioning, toggle behavior, and topmost behavior.

### Earlier

- Date: 2026-03-30
- Prompt summary: Defined the Rainmeter Google Calendar project scope, architecture, and initial milestone plan, then scaffolded the first suite structure and README tracker.
- Status: Completed
- Next context: Continue implementation in the renamed `rainmeter-gcal` and `rainmeter-gcal-flyout` skin folders and keep the README progress log aligned with shipped behavior.
...
- [x] Split the launcher icon and the flyout into separate Rainmeter skins so the collapsed icon no longer owns a large invisible click-blocking window
- [x] Added flyout follow-position behavior, icon-click toggle open/close, and screen-edge-aware flyout placement
- [x] Renamed the working project folders and private repository target to `rainmeter-gcal`
- [x] Removed the old `GoogleCalendar` / `GoogleCalendar Flyout` skin folders so only the renamed skins remain under Rainmeter `Skins`
...
- Folder/file naming: decide whether to also rename internal helper/config filenames such as `GoogleCalendar.config.json` or keep those stable while only the Rainmeter skin folders and repo use `rainmeter-gcal`
...
Do a Rainmeter-side verification pass on the renamed skins: launcher opens/closes the flyout, the flyout follows the launcher, edge-aware placement still works in all four corners, and the topmost behavior is acceptable.
